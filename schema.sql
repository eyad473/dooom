...
-- تحليل الأخطاء (لبناء بروفايل ضعف شخصي)
CREATE TABLE IF NOT EXISTS error_log (
  id INTEGER PRIMARY KEY AUTOINCREMENT,
  chat_id INTEGER NOT NULL,
  category TEXT NOT NULL, -- tenses/prepositions/articles/word_order/vocabulary/spelling/other
  created_at TEXT DEFAULT (datetime('now'))
);
CREATE INDEX IF NOT EXISTS idx_error_chat ON error_log(chat_id, category);

-- منهج أسبوعي مولّد تلقائيًا
